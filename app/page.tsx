...
export default function HomePage() {
  const [activeTab, setActiveTab] = useState<'spot' | 'regular'>('spot')
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      ...
      const facilityIds = [...new Set([
        ...(jobData ?? []).map(j => j.facility_id),
        ...(regularData ?? []).map(j => j.facility_id),
      ].filter(Boolean))]
      ...
      if (facilityIds.length > 0) {
        const { data: facilities } = await supabase
          ...
          .in('id', facilityIds)
        facilities?.forEach(f => { facilityMap[f.id] = f.facility_name })
      }

      setJobs((jobData ?? []).map(j => ({ ...j, facility_name: facilityMap[j.facility_id] ?? '施設' })))
      setRegularJobs((regularData ?? []).map(j => ({ ...j, facility_name: facilityMap[j.facility_id] ?? '施設' })))
      setLoading(false)
    }
    fetchJobs()
  }, [])
  ...
  return (
    <div style={{ fontFamily: "'Noto Sans JP', sans-serif", color: '#1A2235', background: '#FAFAFA' }}>
      ...
      <div style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(160deg, #E0F7FA 0%, #FDF0F0 55%, #FBF7F0 100%)', padding: '32px 16px 0' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 140px', gap: 12, alignItems: 'flex-end', marginBottom: 20 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: '#64748B', lineHeight: 1.8, marginBottom: 6 }}>
              単発バイトから正社員転職まで。<br />現役看護師が作った、看護師目線のプラットフォーム。
            </p>
            <p style={{ fontStyle: 'italic', fontSize: 14, color: '#0096A0' }}>
              — 看護師の経験が、誰かの毎日を支える —
            </p>
          </div>
          <div style={{ position: 'relative', zIndex: 1 }}>
            <Image src="/nurse-hero.jpg" alt="訪問看護師" width={140} height={180}
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14, position: 'relative', zIndex: 1 }}>
          {['📍 那覇市', '浦添市', '宜野湾市', '沖縄市', '名護市', '🏝️ 離島'].map((area, i) => (
            <Link key={area} href="/jobs" style={{ background: i === 0 ? '#0096A0' : '#fff', color: i === 0 ? '#fff' : '#0096A0', border: '1px solid #0096A0', borderRadius: 99, padding: '5px 12px', fontSize: 12, fontWeight: 500, textDecoration: 'none', whiteSpace: 'nowrap' }}>{area}</Link>
          ))}
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 0, position: 'relative', zIndex: 1 }}>
          <Link href="/jobs" style={{ background: '#0096A0', color: '#fff', borderRadius: 12, padding: '13px 8px', fontSize: 13, fontWeight: 700, textAlign: 'center', textDecoration: 'none', display: 'block' }}>🔍 単発求人を探す</Link>
          <Link href="/regular-jobs" style={{ background: '#E07070', color: '#fff', borderRadius: 12, padding: '13px 8px', fontSize: 13, fontWeight: 700, textAlign: 'center', textDecoration: 'none', display: 'block' }}>💼 正社員・パート</Link>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginTop: 16, paddingBottom: 48, position: 'relative', zIndex: 1 }}>
          {[{ num: '260名+', label: '登録看護師' }, { num: '¥0', label: '紹介手数料' }, { num: '全島', label: '沖縄全域対応' }].map(s => (
      ...
      <div style={{ background: '#E0F7FA', padding: '24px 16px' }}>
        <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 4 }}>🗺️ エリアから探す</div>
      ...
      <div style={{ padding: '24px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          ...
          <Link href={activeTab === 'spot' ? '/jobs' : '/regular-jobs'} style={{ fontSize: 12, color: '#E07070', textDecoration: 'none' }}>もっと見る →</Link>
        </div>

        {/* タブ */}
        <div style={{ display: 'flex', background: '#F1F5F9', borderRadius: 10, padding: 3, marginBottom: 14 }}>
          <div onClick={() => setActiveTab('spot')} style={{ flex: 1, textAlign: 'center', padding: '8px', borderRadius: 8, background: activeTab === 'spot' ? '#fff' : 'transparent', color: activeTab === 'spot' ? '#E07070' : '#64748B', fontSize: 13, fontWeight: activeTab === 'spot' ? 600 : 400, cursor: 'pointer', boxShadow: activeTab === 'spot' ? '0 1px 4px rgba(0,0,0,0.08)' : 'none', transition: 'all 0.2s' }}>単発バイト</div>
          <div onClick={() => setActiveTab('regular')} style={{ flex: 1, textAlign: 'center', padding: '8px', borderRadius: 8, background: activeTab === 'regular' ? '#fff' : 'transparent', color: activeTab === 'regular' ? '#0096A0' : '#64748B', fontSize: 13, fontWeight: activeTab === 'regular' ? 600 : 400, cursor: 'pointer', boxShadow: activeTab === 'regular' ? '0 1px 4px rgba(0,0,0,0.08)' : 'none', transition: 'all 0.2s' }}>正社員・パート</div>
        </div>
        ...
        {activeTab === 'spot' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {loading ? (
              <div style={{ textAlign: 'center', padding: 32, color: '#94A3B8' }}>読み込み中...</div>
            ) : jobs.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 32, color: '#94A3B8', fontSize: 13 }}>現在募集中の単発求人はありません</div>
            ) : jobs.map(job => (
              <Link key={job.id} href={`/jobs/${job.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                <div style={{ background: '#fff', borderRadius: 14, overflow: 'hidden', border: '1px solid #EDE0E0' }}>
                  ...
                  <div style={{ padding: '14px 16px' }}>
                    <div style={{ fontSize: 11, color: '#94A3B8', marginBottom: 3 }}>{job.facility_name}</div>
                    <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 8 }}>{job.facility_type} 単発バイト</div>
        ...
        {activeTab === 'regular' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {loading ? (
              ...
              <div style={{ textAlign: 'center', padding: 32, color: '#94A3B8', fontSize: 13 }}>現在募集中の正社員・パート求人はありません</div>
            ) : regularJobs.map(job => (
              <Link key={job.id} href={`/regular-jobs/${job.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                <div style={{ background: '#fff', borderRadius: 14, overflow: 'hidden', border: '1px solid #EDE0E0' }}>
                  <div style={{ height: 6, background: 'linear-gradient(90deg, #0096A0, #1A2235)' }} />
                  <div style={{ padding: '14px 16px' }}>
                    <div style={{ fontSize: 11, color: '#94A3B8', marginBottom: 3 }}>{job.facility_name}</div>
                    <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 8 }}>{job.title}</div>
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
                      <span style={{ background: '#E0F7FA', color: '#0096A0', fontSize: 10, padding: '2px 8px', borderRadius: 99, fontWeight: 600 }}>{job.employment_type}</span>
                      {job.hire_flow === 'interview' && <span style={{ background: '#EDE9FB', color: '#3C3489', fontSize: 10, padding: '2px 8px', borderRadius: 99, fontWeight: 600 }}>💬 面談型</span>}
                      {job.location && <span style={{ background: '#F1F5F9', color: '#64748B', fontSize: 10, padding: '2px 8px', borderRadius: 99, fontWeight: 600 }}>📍 {job.location}</span>}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: 20, fontWeight: 700, color: '#0096A0' }}>¥{job.salary_amount?.toLocaleString()} <span style={{ fontSize: 11, color: '#94A3B8', fontWeight: 400 }}>{job.salary_type}〜</span></span>
                      <span style={{ background: '#0096A0', color: '#fff', borderRadius: 99, padding: '6px 16px', fontSize: 12, fontWeight: 600 }}>詳細を見る</span>
                    </div>
                  </div>
                </div>
              </Link>
      ...
      <div style={{ position: 'relative', overflow: 'hidden', margin: '0 16px 24px', borderRadius: 16 }}>
        <Image src="/nurse-hero.jpg" alt="訪問看護の現場" width={400} height={240}
      ...
      <div style={{ padding: '24px 16px' }}>
        ...
        <Link href="/register" style={{ display: 'block', textDecoration: 'none', marginTop: 20 }}>
          <div style={{ background: '#E07070', color: '#fff', textAlign: 'center', padding: 14, borderRadius: 12, fontSize: 15, fontWeight: 700 }}>👩‍⚕️ 無料で登録する</div>
        </Link>
      </div>
      ...
      <div style={{ background: '#1A2235', padding: '24px 16px' }}>
        ...
        <Link href="/for-facility" style={{ display: 'block', textDecoration: 'none' }}>
          <div style={{ border: '1px solid rgba(255,255,255,0.3)', color: '#fff', textAlign: 'center', padding: 13, borderRadius: 12, fontSize: 14, fontWeight: 600 }}>🏥 施設向けページを見る</div>
        </Link>
      </div>
      ...
      <div style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(135deg, #0096A0 0%, #006070 50%, #1A2235 100%)', padding: '32px 16px', textAlign: 'center' }}>
        ...
        <div style={{ fontStyle: 'italic', fontSize: 20, color: 'rgba(255,255,255,0.9)', marginBottom: 8 }}>めんそーれ、irodori+へ</div>
        ...
        <Link href="/register" style={{ display: 'block', textDecoration: 'none', marginBottom: 10 }}>
          <div style={{ background: '#fff', color: '#0096A0', borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 700 }}>👩‍⚕️ 無料で看護師登録する</div>
        </Link>
        <Link href="/for-facility" style={{ display: 'block', textDecoration: 'none' }}>
          <div style={{ background: '#E07070', color: '#fff', borderRadius: 12, padding: 12, fontSize: 14, fontWeight: 600 }}>🏥 施設向けページを見る</div>
        </Link>
      </div>
